...
bool calcRayBoxIntersection(const tfluids_(vec3)& pos,
  ...
  real box_min[3];
  box_min[0] = ctr.x - (real)(0.5f) - hit_margin;
  box_min[1] = ctr.y - (real)(0.5f) - hit_margin;
  box_min[2] = ctr.z - (real)(0.5f) - hit_margin;
  real box_max[3];
  box_max[0] = ctr.x + (real)(0.5f) + hit_margin;
  box_max[1] = ctr.y + (real)(0.5f) + hit_margin;
  box_max[2] = ctr.z + (real)(0.5f) + hit_margin;
...
bool calcLineTrace(const tfluids_(vec3)& pos, const tfluids_(vec3)& delta,
                   ...
                   tfluids_(vec3)* new_pos, at::Tensor& exit, const bool do_line_trace) {
  ...
  (*new_pos) = pos;
  at::Tensor delta_ten = CPU(at::kFloat).zeros({3});
  delta_ten[0] = delta.x;
  delta_ten[1] = delta.y;
  delta_ten[2] = delta.z;

  const real length = at::Scalar(delta_ten.norm()).toFloat();
    exit[0] = length;
    //exit[1] = delta.y;
    //exit[2] = delta.z;
