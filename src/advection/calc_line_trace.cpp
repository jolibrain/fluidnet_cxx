...
const float hit_margin = static_cast<float>(1e-5);
const float epsilon = static_cast<float>(1e-12);
...
void ClampToDomainReal(
  ...
  pos.x = std::min<float>(std::max<float>(pos.x, hit_margin), 
                         (float)flags.xsize() - hit_margin);
  pos.y = std::min<float>(std::max<float>(pos.y, hit_margin),
                         (float)flags.ysize() - hit_margin);
  pos.z = std::min<float>(std::max<float>(pos.z, hit_margin),
                         (float)flags.zsize() - hit_margin);
}
...
bool HitBoundingBox(const float* minB, const float* maxB,  // box
  ...
  for (i = 0; i < 3; i++) {
    if (quadrant[i] != MIDDLE && dir[i] != (float)(0.0)) {
  ...
  // not).
  if (maxT[whichPlane] < (float)(0.0)) {
    return false;
  }

  const float err_tol = 1e-6;
  for (i = 0; i < 3; i++) {
    if (whichPlane != i) {
...
bool calcRayBoxIntersection(const vec3& pos,
  ...
  float box_min[3];
  box_min[0] = ctr.x - (float)(0.5) - hit_margin;
  box_min[1] = ctr.y - (float)(0.5) - hit_margin;
  box_min[2] = ctr.z - (float)(0.5) - hit_margin;
  float box_max[3];
  box_max[0] = ctr.x + (float)(0.5) + hit_margin;
  box_max[1] = ctr.y + (float)(0.5) + hit_margin;
  box_max[2] = ctr.z + (float)(0.5) + hit_margin;
...
bool calcRayBorderIntersection(const vec3& pos,
  ...
  if (next_pos.x <= hit_margin) {  // left face.
    ...
    if (std::abs(dx) >= epsilon) {
      ...
      min_step = std::min<float>(min_step, xstep);
    }
  }
  if (next_pos.y <= hit_margin) {
    ...
    if (std::abs(dy) >= epsilon) {
      ...
      min_step = std::min<float>(min_step, ystep);
    }
  }
  if (next_pos.z <= hit_margin) {
    ...
    if (std::abs(dz) >= epsilon) {
      ...
      min_step = std::min<float>(min_step, zstep);
    }
  }
  ...
  if (min_step < 0 || min_step >= std::numeric_limits<float>::max()) {
    return false;
  }
...
bool calcLineTrace(const vec3& pos, const vec3& delta,
  ...
  float cur_length = (float)0;
  vec3 next_pos;  // Tentative step location.
  while (cur_length < (length - hit_margin)) {
    // We haven't stepped far enough. So take a step.
    float cur_step = std::min<float>(length - cur_length, (float)(1));
    ...
    if (IsBlockedCellReal(flags, next_pos, ibatch)) {
      ...
      for (uint32_t count = 0; count <= max_count; count++) {
        ...
        next_pos_ctr.x = (float)(ix) + (float)0.5;
        next_pos_ctr.y = (float)(iy) + (float)0.5;
        next_pos_ctr.z = (float)(iz) + (float)0.5;
