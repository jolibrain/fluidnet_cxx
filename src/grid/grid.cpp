...
namespace fluid {
  ...
  T GridBase::getSize() const {
     T size = getType(bckd, at::kInt).zeros({3});
     size[0] = xsize();
     size[1] = ysize();
     size[2] = zsize();
     return size;
  ...
  T RealGrid::interpol(const T& pos, T b) const {
    T xi = getType(bckd, at::kInt).zeros({3});
    T stf0 = getType(bckd, real).zeros({3}); 
    T stf1 = getType(bckd, real).zeros({3}); 
  ...
  T RealGrid::interpolWithFluid(
    ...
    if (is_3d()) {
      ...
      const T p_a = getType(bckd, at::kInt).zeros({3});
      const T p_b = getType(bckd, at::kInt).zeros({3});
      p_a[0] = xi[0];
      p_a[1] = xi[1];
      p_a[2] = xi[2];
      ...
      const T p_c = getType(bckd, at::kInt).zeros({3});
      const T p_d = getType(bckd, at::kInt).zeros({3});
      ...
      const T p_e = getType(bckd, at::kInt).zeros({3});
      const T p_f = getType(bckd, at::kInt).zeros({3});
      ...
      const T p_g = getType(bckd, at::kInt).zeros({3});
      const T p_h = getType(bckd, at::kInt).zeros({3});
  ...
  const T MACGrid::getCentered(T i, T j, T k, T b) const {  
    ...
    const T x = getType(bckd, real).zeros({3});
