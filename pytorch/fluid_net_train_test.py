...
# config module is "global" as other modules will be able to load it
...
class FluidNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x[:,1,:,:,:] = UDiv.select(1,1)
        x[:,2,:,:,:] = flags.squeeze(1)
        #show_data = torch.squeeze(x[0,2,:,:,:].cpu()).data.numpy()
        #plt.imshow(show_data,origin='lower')
        #plt.show(block=True)
        ...
        #y0 =torch.squeeze(x0[0][0].cpu()).data.numpy()
        #y1 =torch.squeeze(x1[0][0].cpu()).data.numpy()
        #y2 =torch.squeeze(x2[0][0].cpu()).data.numpy()
        #fig,axes = plt.subplots(nrows = 1, ncols =3 )
        #axes[0].imshow(y0,origin='lower')
        #axes[0].set_title('x0')
        #axes[0].axis('off')
        #axes[1].imshow(y1,origin='lower')
        #axes[1].set_title('x1')
        #axes[1].axis('off')
        #axes[2].imshow(y2,origin='lower')
        #axes[2].set_title('x2')
        #axes[2].axis('off')
        #plt.show(block=True)
...
scheduler = ReduceLROnPlateau(optimizer, mode = 'min',factor = 0.4, patience = 10, verbose = True, threshold = 3e-4, threshold_mode = 'rel')
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate (train_loader):
        ...
        #show_data = torch.squeeze(err[0,0,:,:,:].cpu()).data.numpy()
        #plt.imshow(show_data,origin='lower')
        #plt.show(block=True)
        #show_data =torch.squeeze(data[0][0].cpu()).data.numpy()
        #show_truth =torch.squeeze(target[0].cpu()).data.numpy()
        #fig,axes = plt.subplots(nrows = 1, ncols =2 )
        #axes[0].imshow(show_data,origin='lower')
        #axes[0].set_title('Input')
        #axes[0].axis('off')
        #axes[1].imshow(show_truth,origin='lower')
