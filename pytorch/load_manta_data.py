...
def loadMantaFile(fname):
    ...
    with open(fname, 'rb') as f:
        ...
        Ux = torch.FloatTensor(struct.unpack('f'*numel, f.read(4*numel)))
        Uy = torch.FloatTensor(struct.unpack('f'*numel, f.read(4*numel)))
        if (is3D):
            Uz = torch.FloatTensor(struct.unpack('f'*numel, f.read(4*numel)))

        p = torch.FloatTensor(struct.unpack('f'*numel, f.read(4*numel)))
