...
def addBuoyancy(U, flags, density, gravity, dt):
    ...
    fluid100 = zeroBy.where( i <= 0, (flags[idx_b, zero, k, j, i-1].eq(CellType.TypeFluid))).__and__(mCont)
    factor = 0.5 * strength[0] * (density.squeeze(1) + \
        zero_f.where(i <= 0, density[idx_b, zero, k, j, i-1]) ) / density.squeeze(1)
    ...
    factor = 0.5 * strength[1] * (density.squeeze(1) + \
        zero_f.where( j <= 0, density[idx_b, zero, k, j-1, i]) ) / density.squeeze(1)
    ...
    if (is3D):
        ...
        factor = 0.5 * strength[2] * (density.squeeze(1) + \
            zero_f.where(k <= 1, density[idx_b, zero, k-1, j, i]) ) / density.squeeze(1)
