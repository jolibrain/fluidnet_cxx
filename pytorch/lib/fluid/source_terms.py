...
# @input T - scalar temperature grid.
# @input gravity - 3D vector indicating direction of gravity.
# @input dt - scalar timestep.

def addTemperatureBuoyancy(U, flags, T, gravity, dt):
    cuda = torch.device('cuda')
    # Argument check
    assert U.dim() == 5 and flags.dim() == 5 and T.dim() == 5,\
        "Dimension mismatch"
    ...
    assert T.is_same_size(flags), "Size mismatch"

    assert U.is_contiguous() and flags.is_contiguous() and \
        T.is_contiguous(), "Input is not contiguous"
    ...
    if (is3D):
        ...
        factor = strength[2] *(0.5* (T.squeeze(1) + \
            zero_f.where(k <= 1, T[idx_b, zero, k-1, j, i]) ))
...
def addDensityBuoyancy(U, flags, density, gravity, dt):
    cuda = torch.device('cuda')
    # Argument check
