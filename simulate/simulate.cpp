...
void simulate(std::vector<at::Tensor>& batch, int res) {
  ...
  at::Tensor pCPU =     p.toBackend(at::Backend::CPU);
  at::Tensor divCPU =   div.toBackend(at::Backend::CPU);
  at::Tensor flagsCPU = flags.toBackend(at::Backend::CPU);
 
  residual = fluid::solveLinearSystemJacobi(pCPU, flagsCPU, divCPU, is3D, pTol, maxIter, false);
  
  p = pCPU.toBackend(at::Backend::CUDA);
