...
#include "grid.h"
    ...
    #define toRealData toFloatData
#endif
#ifdef TH_REAL_IS_DOUBLE
    #define toRealData toDoubleData
#endif

tfluids_(GridBase)::tfluids_(GridBase)(at::Tensor* grid, bool is_3d) :
     is_3d_(is_3d), tensor_(grid), p_grid_(grid->toRealData()) {
...
tfluids_(FlagGrid)::tfluids_(FlagGrid)(at::Tensor* grid, bool is_3d) :
...
tfluids_(RealGrid)::tfluids_(RealGrid)(at::Tensor* grid, bool is_3d) :
    tfluids_(GridBase)(grid, is_3d) {
  if (nchan() != 1) {
...
real tfluids_(RealGrid)::interpolWithFluid(
  ...
  if (is_3d()) {
    ...
    ///                    data(p_b, 0, ibatch), flags.isFluid(p_b, ibatch),
    ///                    t0, t1, &is_fluid_ab, &val_ab);
  real val_a = data(p_a, 0, ibatch); 
  real val_b = data(p_b, 0, ibatch); 
  bool is_fluid_a = flags.isFluid(p_a, ibatch); 
  bool is_fluid_b = flags.isFluid(p_b, ibatch);
  real temp = 0;
  if (!is_fluid_a && !is_fluid_b) {
    val_ab = (real)0;
    is_fluid_ab = false;
...
tfluids_(MACGrid)::tfluids_(MACGrid)(at::Tensor* grid, bool is_3d) :
    tfluids_(GridBase)(grid, is_3d) {
  if (nchan() != 2 && nchan() != 3) {
...
tfluids_(VecGrid)::tfluids_(VecGrid)(at::Tensor* grid, bool is_3d) :
    tfluids_(GridBase)(grid, is_3d) {
  if (nchan() != 2 && nchan() != 3) {
